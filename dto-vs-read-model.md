...
│   └── queries/
│       ├── read-models/
│       │   └── user-profile.ts         # UserProfileReadModel
│       └── get-user-profile/
│           ├── query.ts
│           └── handler.ts
└── infrastructure/
...
│   │   ├── read-models/              # ★ READ MODELS VIVEN AQUÍ (carpeta compartida)
│   │   │   ├── user-profile.ts       # UserProfileReadModel
│   │   │   └── order-details.ts      # OrderDetailsReadModel
│   │   │
│   │   └── get-user-profile/         # Cada query solo tiene query.ts y handler.ts
│   │       ├── query.ts
│   │       └── handler.ts
│   │
│   └── events/
│
├── infra/                            # ← Persistencia y servicios externos
│   ├── persistence/
│   │   ├── repositories/
...
### 1. Read Models → `app/queries/read-models/`
...
```
app/queries/
├── read-models/                      # ★ Carpeta compartida para TODOS los read models
│   ├── order-details.ts              # OrderDetailsReadModel
│   ├── user-profile.ts               # UserProfileReadModel
│   └── content-item.ts               # ContentItemReadModel
│
├── get-order-details/                # Cada query solo tiene query.ts y handler.ts
│   ├── query.ts
│   └── handler.ts
│
└── get-user-profile/
...
- ✅ `content-item.ts` → `ContentItemReadModel` (correcto)
- ✅ `order-details.ts` → `OrderDetailsReadModel` (correcto)
- ❌ `read-model.ts` → incorrecto (nombre genérico)
- ❌ `content-item-read-model.ts` → incorrecto (sufijo redundante)
...
### 3. Projections → `infra/persistence/projections/`
...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         ↓
┌─────────────────────────────────────────────────────────────────┐
│ app/queries/get-order-details/handler.ts                        │
│                                                                   │
...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         ↓
┌─────────────────────────────────────────────────────────────────┐
│ api/http/dtos/responses/order-details.ts                        │
│                                                                   │
│ interface OrderDetailsResponseDto {                             │
│   orderId: string;                                              │
│   customerName: string;                                         │
...
| ----------------- | ------------------------------------- | --------------------------------------- |
| **Read Model**    | `app/queries/read-models/`            | Define QUÉ datos necesita la aplicación |
| **Query**         | `app/queries/<query-name>/query.ts`   | Define la query                         |
| **Query Handler** | `app/queries/<query-name>/handler.ts` | Orquesta la obtención del Read Model    |
| **Projection**    | `infra/persistence/projections/`      | CÓMO construir el Read Model (DB)       |
...
├── read-models/              # ★ Carpeta compartida para TODOS los read models
│   ├── content-item.ts       # ContentItemReadModel
│   └── ingestion-job.ts      # IngestionJobReadModel
│
├── get-content-by-hash/      # Cada query solo tiene query.ts y handler.ts
│   ├── query.ts
│   └── handler.ts
│
└── get-job-by-id/
...
- **Read Models compartidos** en `app/queries/read-models/` evitan duplicación
