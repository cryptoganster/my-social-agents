...
import { UpdateSourceHealthOnJobFailed } from '../update-source-health-on-job-failed';
...
describe('UpdateSourceHealthOnJobFailed', () => {
  let handler: UpdateSourceHealthOnJobFailed;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<UpdateSourceHealthOnJobFailed>(
      UpdateSourceHealthOnJobFailed,
    );
  ...
  describe('handle', () => {
    it('should execute UpdateSourceHealthCommand with failure outcome when JobFailed is received', async () => {
      // Arrange
      const event = new JobFailed(
        'job-123',
        'source-456',
        {
    ...
    it('should log error details when JobFailed is received', async () => {
      // Arrange
      const event = new JobFailed(
        'job-123',
        'source-456',
    ...
    it('should log error and continue if command execution fails', async () => {
      ...
      expect(commandBus.execute).toHaveBeenCalledTimes(1);
      expect(loggerErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error handling JobFailed'),
        expect.any(String),
      );
    });
