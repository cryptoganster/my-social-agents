...
import { UpdateSourceHealthOnJobCompleted } from '../update-source-health-on-job-completed';
...
describe('UpdateSourceHealthOnJobCompleted', () => {
  let handler: UpdateSourceHealthOnJobCompleted;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<UpdateSourceHealthOnJobCompleted>(
      UpdateSourceHealthOnJobCompleted,
    );
  ...
  describe('handle', () => {
    it('should execute UpdateSourceHealthCommand with success outcome when JobCompleted is received', async () => {
      // Arrange
      const event = new JobCompleted(
        'job-123',
        'source-456',
        {
          itemsCollected: 50,
    ...
    it('should log error and continue if command execution fails', async () => {
      ...
      expect(loggerErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error handling JobCompleted'),
        expect.any(String),
      );
    });
